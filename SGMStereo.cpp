...
void SGMStereo::allocateDataBuffer() {
	...
	std::cerr << totalBufferSize_*sizeof(short) << std::endl;
	
	std::cerr << pathMinCostBufferSize_ << std::endl;
	std::cerr << pathCostBufferSize_ << std::endl;
	std::cerr << costSumBufferSize_ << std::endl;

	sgmBuffer_ = reinterpret_cast<short*>(_mm_malloc(totalBufferSize_*sizeof(short), 16));
	//sgmBuffer_ = reinterpret_cast<short*>(_mm_malloc(539245664, 16));

	std::cerr << sgmBuffer_ << std::endl;
}
...
void SGMStereo::computeCostImage(const png::image<png::rgb_pixel>& leftImage, const png::image<png::rgb_pixel>& rightImage) {
	...
	std::cerr << "OK SGMStereo computeCostImage() line 186" << std::endl;
	computeLeftCostImage(leftGrayscaleImage, rightGrayscaleImage);
	std::cerr << "OK SGMStereo computeCostImage() line 188" << std::endl;
	computeRightCostImage();
	std::cerr << "OK SGMStereo computeCostImage() line 190" << std::endl;
...
void SGMStereo::computeLeftCostImage(const unsigned char* leftGrayscaleImage, const unsigned char* rightGrayscaleImage) {
	...
	std::cerr << "OK SGMStereo computeLeftCostImage() line 234" << std::endl;
	calcRowCosts(leftSobelRow, leftCensusRow,
				 rightSobelRow, rightCensusRow,
				 costImageRow);
	std::cerr << "OK SGMStereo computeLeftCostImage() line 238" << std::endl;

	_mm_free(leftSobelImage);
	_mm_free(rightSobelImage);
	free(leftCensusImage);
	std::cerr << "OK SGMStereo computeLeftCostImage() line 243" << std::endl;
	free(rightCensusImage);
	std::cerr << "OK SGMStereo computeLeftCostImage() line 245" << std::endl;
}
							 ...
							 unsigned short* costImageRow)
{
	...
	std::cerr << "OK SGMStereo calcRowCosts() befor 3*for line 345" << std::endl;
	...
	std::cerr << "OK SGMStereo calcRowCosts() after 3*for line 416" << std::endl;
}
...
void SGMStereo::computeRightCostImage() {
	...
	for (int y = 0; y < height_; ++y) {
		...
		for (int x = width_ - disparityTotal_ + 1; x < width_; ++x) {
			...
			//std::cerr << "OK SGMStereo computeCostImage() line 604 !!!!!!!" << std::endl;
			unsigned short lastValue = *(rightCostRow + disparityTotal_*x + maxDisparityIndex - 1);
			//unsigned short lastValue = *rightCostRow + disparityTotal_*x + maxDisparityIndex - 1;
			//std::cerr << "OK SGMStereo computeCostImage() line 606 !!!!!!!" << std::endl;
			...
			for (int d = maxDisparityIndex; d < disparityTotal_; ++d) {
				//std::cerr << "OK SGMStereo computeCostImage() line611" << std::endl;
				*(rightCostPointer) = lastValue;
				//std::cerr << "OK SGMStereo computeCostImage() line613" << std::endl;
				++rightCostPointer;
			}
			\
		}
...
void SGMStereo::performSGM(unsigned short* costImage, unsigned short* disparityImage) {
	...
	std::cerr << "OK SGMStereo performSGM() line 610" << std::endl;

	std::cerr << costSums << std::endl; 

	std::cerr << costSumBufferSize_*sizeof(short) << std::endl;

	memset(costSums, 0, costSumBufferSize_*sizeof(short));
	std::cerr << "OK SGMStereo performSGM() line 612" << std::endl;
